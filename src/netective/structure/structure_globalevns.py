...
def get_child_classes(parent_class, selected_props, user_globals=False) -> dict:
    child_classes = {}
    all_properties = []
    ...
    if user_globals:
        global_namespace = user_globals# inspect.stack()[1][0].f_globals
        ...
        for name, obj in global_namespace.items():
            if inspect.isclass(obj) and issubclass(obj, parent_class) and obj != parent_class:
                print(f'{name} is a used defined property')
                property_childs.append((obj.__name__, obj))
    
    # print(f"Properties used for analysis: ") #TODO: UX: only if vervose
    if selected_props == "all":
        for name, obj in property_childs:
            if inspect.isclass(obj) and issubclass(obj, parent_class) and obj != parent_class:
...
class Structure:
    ...
    def get_props(
        self, props: str | list = "all", child_classes: list = None, user_globals: bool = False
        ...
        Args:
            ...
            user_globals: bool.
                pass globals() if you want your own classes to be considered.
        ...
        if (
            ...
            try:
                ...
                if child_classes is None:
                    child_classes = get_child_classes(PARENT_CLASS, props, user_globals=user_globals)
                    print(child_classes)
...
def characterize_network(
    ...
    if child_classes is not None:
        ...
        scalar_values, dist_values = struc.get_props(props=selected_props, user_globals=user_globals)
...
def compare_structure(
    ...
    child_classes = get_child_classes(PARENT_CLASS, selected_props, user_globals=user_globals)
